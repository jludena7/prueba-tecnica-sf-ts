...
describe('MovieTheaterService', () => {
    ...
    beforeEach(() => {
        ...
        movieTheaterService = new MovieTheaterService(movieTheaterRepository, filmService);
    });
    ...
    it('should return a MovieTheaterPremiereEntity if movie theater is found', async () => {
        ...
        (movieTheaterRepository.get as jest.Mock).mockResolvedValue(mockMovieTheaterEntity);
        (filmService.getFilmsByIds as jest.Mock).mockResolvedValue(mockFilmColumnEs);
    ...
    it('should throw NotFoundError if no movie theater is found', async () => {
        ...
        await expect(movieTheaterService.get(inputData)).rejects.toThrow(new NotFoundError());
    });
});
